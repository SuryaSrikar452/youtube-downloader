...
async function downloadStream(videoId, format, res) {
  ...
  if (format === 'mp3') {
    ...
    const buildArgs = (playerClient, cookiesPathToUse) => {
      const args = [
        '-o', tempAudioPath,
        '-x',
        '--audio-format', 'mp3',
      ...
      if (cookiesPathToUse) args.push('--cookies', cookiesPathToUse);
    ...
    try {
      ...
      await runYtDlp(ytDlpPath, buildArgs('android', null), tempAudioPath, res);
      success = true;
      ...
      try {
        ...
        if (freshCookiesPath) {
          tempCookiesPath = path.join(os.tmpdir(), `cookies_download_mp3_${videoId}_${Date.now()}.txt`);
        ...
        const attempt2Args = buildArgs('web,tv', tempCookiesPath);
        console.log(`[ytEngine] Spawning yt-dlp with exact command array:`, [ytDlpPath, ...attempt2Args]);

        console.log(`[ytEngine] Retrying MP3 download for videoId: ${videoId} using client: web,tv, cookies: ${!!tempCookiesPath}`);
        ...
        if (tempCookiesPath && fs.existsSync(tempCookiesPath)) {
          try {
            ...
            console.log(`[ytEngine] Cleaned up temporary request-scoped cookie file: ${tempCookiesPath}`);
          } catch (e) {
            console.error(`[ytEngine] Failed to delete temp cookie file ${tempCookiesPath}:`, e);
          }
        }
      }
    }
    ...
    try {
      ...
      await runYtDlp(ytDlpPath, buildArgs('android', null), tempVideoPath, res);
      success = true;
      activeClient = 'android';
      cookiesUsed = false;
    } catch (err1) {
      ...
      try {
        const freshCookiesPath = cookies.getCookiesPath();
        if (freshCookiesPath) {
          tempCookiesPath = path.join(os.tmpdir(), `cookies_download_mp4_${videoId}_${Date.now()}.txt`);
          fs.copyFileSync(freshCookiesPath, tempCookiesPath);
        }
        ...
        console.log(`[ytEngine] Retrying MP4 download for videoId: ${videoId} using client: web,tv, cookies: ${!!tempCookiesPath}`);
