...
async function getInnertube() {
  if (!yt) {
    ...
    yt.session.on('auth', (data) => {
      console.log('[ytEngine Auth] Google Sign-In Successful!');
...
async function getGoogleStatus() {
  const innertube = await getInnertube();
  return {
    isLoggedIn: innertube.session.logged_in,
    currentAuthData
  };
}
...
async function getStreamWithFallback(innertube, videoId, options) {
  try {
    ...
    if (err.message && err.message.includes('LOGIN_REQUIRED')) {
      console.log(`[ytEngine] ANDROID client login required, trying WEB client with session...`);
