 ...
 * Auto-falls back to no cookies if process.env.YOUTUBE_COOKIES contains expired session cookies.
 */
  ...
  if (format === 'mp3') {
    ...
    const buildArgs = (useCookies) => {
      const args = [
        ...
        '--extractor-args', 'youtube:player_client=ios,android,tv_embedded,web_creator',
      ];
    ...
    try {
      ...
      if (cookiesPath && (firstErr.message.includes('cookies') || firstErr.message.includes('sign in'))) {
        console.warn('[ytEngine] Cookies appear expired/invalid. Retrying WITHOUT cookies...');
