...
// Helper to download stream with client fallback (ANDROID -> YTMUSIC)
async function getStreamWithFallback(innertube, videoId, options) {
  try {
    ...
    if (err.message && err.message.includes('LOGIN_REQUIRED')) {
      console.log(`[ytEngine] ANDROID client login required, falling back to YTMUSIC client...`);
      return await innertube.download(videoId, { ...options, client: 'YTMUSIC' });
    }
    throw err;
  }
}
