 ...
 * Uses ios,android player client chain with skip=webpage,configs,js to bypass datacenter HTTP 429 webpage blocks.
 */
  ...
  if (format === 'mp3') {
    ...
    const buildArgs = (useCookies) => {
      const args = [
        ...
        '--extractor-args', 'youtube:player_client=ios,android;skip=webpage,configs,js',
      ];
    ...
    try {
      ...
      if (cookiesPath || firstErr.message.includes('429') || firstErr.message.includes('sign in')) {
