 ...
 * Uses ios,android player client fallback chain with skip=webpage to bypass datacenter HTTP 429 webpage blocks.
 */
  ...
  if (format === 'mp3') {
    ...
    const buildArgs = (useCookies) => {
      const args = [
        ...
        '--extractor-args', 'youtube:player_client=ios,android;skip=webpage',
      ];
